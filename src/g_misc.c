...
//	VectorSet (self->maxs, 16, 16, 16);
	...
	gib->velocity[0] = random()*80 -40;
	gib->velocity[1] = random()*80 -40;
	gib->velocity[2] = random()*50;
...
//	gib->avelocity[2] = random()*400;
	...
	gib->think = Remove_Gib;//faf G_FreeEdict;
...
void ThrowHead (edict_t *self, char *gibname, int damage, int type)
{
	...
	self->think = Remove_Gib;//faf G_FreeEdict;
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	chunk->think = Remove_Gib;//faf G_FreeEdict;
...
void HeadShotGib (edict_t *ent, vec3_t head_pos, vec3_t forward, vec3_t right, vec3_t player_vel)
{
	...
	gib->nextthink = level.time + 45.0; //45 seconds
